...
LinyapsContext *linyaps_context_new(void) {
  ...
  r = sd_bus_add_match(ctx->bus, &ctx->task_added_slot,
                       "type='signal',sender='" PM_DEST "',interface='" PM_IFACE
                       "',member='TaskAdded'",
                       on_task_added, ctx);
  if (r < 0) {
...
bool linyaps_is_service_available(LinyapsContext *ctx) {
  ...
  int r =
      sd_bus_call_method(ctx->bus, PM_DEST, PM_PATH,
...
void linyaps_search(LinyapsContext *ctx, const char *keyword, const char *repos,
  ...
  sd_bus_message *reply = NULL;
  sd_bus_error err = SD_BUS_ERROR_NULL;
  int r = sd_bus_message_new_method_call(ctx->bus, &m, PM_DEST, PM_PATH,
                                         PM_IFACE, "Search");
  if (r >= 0)
  ...
  if (r < 0) {
    LOG_ERR("dbus", "Search D-Bus 调用失败: %s",
  ...
  if (r < 0 || code != 0 || !job_id || !*job_id) {
    if (cb)
      cb(NULL, 0, r < 0 ? r : (code != 0 ? (int)code : -EINVAL),
...
static void call_task_method(LinyapsContext *ctx, const char *method,
                             const char *app_id, const char *version,
                             const char *channel, LinyapsProgressCallback cb,
                             void *userdata) {
  LOG_INFO("dbus", "%s: id=%s ver=%s ch=%s", method, app_id ? app_id : "(null)",
  ...
  if (r < 0) {
    ...
    if (pr < 0 || code != 0) {
      LOG_ERR("dbus", "%s 返回错误: code=%lld msg=%s", method, (long long)code,
              message ? message : "");
      pending_pop_tail(ctx);
      notify_failed(cb, userdata, pr < 0 ? strerror(-pr) : message,
                    pr < 0 ? pr : (int)code);
    } else {
...
void linyaps_install(LinyapsContext *ctx, const char *app_id,
                     const char *version, const char *channel,
                     LinyapsProgressCallback cb, void *userdata) {
  call_task_method(ctx, "Install", app_id, version, channel, cb, userdata);
}

void linyaps_uninstall(LinyapsContext *ctx, const char *app_id,
                       const char *version, const char *channel,
                       LinyapsProgressCallback cb, void *userdata) {
...
/* ------------------------------------------------------------------ */
/* Update (D-Bus with packages array + appOnly + depsOnly + channel)  */
/* ------------------------------------------------------------------ */

void linyaps_update(LinyapsContext *ctx, const char *app_id,
                    const char *channel, LinyapsProgressCallback cb,
                    void *userdata) {
  LOG_INFO("dbus", "Update: id=%s ch=%s", app_id ? app_id : "(null)",
           channel ? channel : "-");
  if (!ctx || !ctx->bus || !app_id || !*app_id) {
    notify_failed(cb, userdata, "invalid package request", -EINVAL);
    return;
  ...
  if (r >= 0) {
    ...
    if (r >= 0)
      r = sd_bus_message_append_basic(m, 's', "packages");
    if (r >= 0)
    ...
    if (r >= 0)
      r = sd_bus_message_open_container(m, 'a', "{sv}");
    if (r >= 0)
      r = append_sv_string(m, "id", app_id);
    if (channel && *channel)
      if (r >= 0)
        r = append_sv_string(m, "channel", channel);
    if (r >= 0)
      r = sd_bus_message_close_container(m); /* close a{sv} */
    if (r >= 0)
      r = sd_bus_message_close_container(m); /* close v(a{sv}) */
    if (r >= 0)
      r = sd_bus_message_close_container(m); /* close a(v) */
    if (r >= 0)
      r = sd_bus_message_close_container(m); /* close v(av) */
    if (r >= 0)
      r = sd_bus_message_close_container(m); /* close dict entry */
  }
  /* appOnly: false */
  if (r >= 0)
    r = append_sv_bool(m, "appOnly", 0);
  /* depsOnly: false */
  if (r >= 0)
    r = append_sv_bool(m, "depsOnly", 0);
  if (r >= 0)
    r = sd_bus_message_close_container(m); /* close a{sv} */
  if (r >= 0)
    r = sd_bus_call(ctx->bus, m, CALL_TIMEOUT_USEC, &err, &reply);
  if (r < 0) {
    LOG_ERR("dbus", "Update 调用失败: %s",
            err.message ? err.message : strerror(-r));
    pending_pop_tail(ctx);
    notify_failed(cb, userdata, err.message ? err.message : strerror(-r), r);
  } else {
    ...
    int pr = parse_common_result(reply, &code, &message);
    if (pr < 0 || code != 0) {
      LOG_ERR("dbus", "Update 返回错误: code=%lld msg=%s", (long long)code,
              message ? message : "");
      pending_pop_tail(ctx);
      notify_failed(cb, userdata, pr < 0 ? strerror(-pr) : message,
                    pr < 0 ? pr : (int)code);
...
void linyaps_cancel_task(LinyapsContext *ctx, const char *task_object_path) {
  ...
  sd_bus_error err = SD_BUS_ERROR_NULL;
  sd_bus_message *reply = NULL;
  sd_bus_call_method(ctx->bus, PM_DEST, task_object_path, TASK_IFACE, "Cancel",
                     &err, &reply, "");
  sd_bus_error_free(&err);
  sd_bus_message_unref(reply);
}
...
void linyaps_reply_interaction(LinyapsContext *ctx,
                               const char *task_object_path, bool accepted) {
  if (!ctx || !ctx->bus || !task_object_path)
    return;
  sd_bus_message *m = NULL;
  sd_bus_message *reply = NULL;
  ...
  int r = sd_bus_message_new_method_call(
      ctx->bus, &m, PM_DEST, task_object_path, TASK_IFACE, "ReplyInteraction");
  if (r >= 0)
    r = sd_bus_message_open_container(m, 'a', "{sv}");
  if (r >= 0)
...
void linyaps_prune(LinyapsContext *ctx, LinyapsCompletionCallback cb,
                   void *userdata) {
  if (!ctx || !ctx->bus) {
    if (cb)
  ...
  sd_bus_error err = SD_BUS_ERROR_NULL;
  sd_bus_message *reply = NULL;
  int r = sd_bus_call_method(ctx->bus, PM_DEST, PM_PATH, PM_IFACE, "Prune",
                             &err, &reply, "");
  int64_t code = r;
  char *message = NULL;
  if (r >= 0)
  ...
  if (cb)
    cb(r < 0 ? r : (int)code, r < 0 ? strerror(-r) : (message ? message : ""),
       userdata);
  free(message);
  sd_bus_error_free(&err);
  sd_bus_message_unref(reply);
