...
static CURL *curl_pool_acquire(struct curl_slist **out_headers) {
  ...
  for (int i = 0; i < CURL_POOL_SIZE; i++) {
    if (!g_curl_pool[i].in_use) {
      ...
      if (!g_curl_pool[i].handle) {
        ...
        /* 预配置公共 headers */
        g_curl_pool[i].headers = NULL;
        g_curl_pool[i].headers = curl_slist_append(
            g_curl_pool[i].headers, "Content-Type: application/json");
        g_curl_pool[i].headers = curl_slist_append(g_curl_pool[i].headers,
  ...
  if (curl) {
    *out_headers = NULL;
    *out_headers =
        curl_slist_append(*out_headers, "Content-Type: application/json");
...
/* 归还 CURL handle 到池中 */
static void curl_pool_release(CURL *curl, struct curl_slist *headers,
                              bool is_pooled) {
  if (!curl)
    return;
...
/* 销毁连接池（程序退出时调用） */
__attribute__((destructor)) static void curl_pool_destroy(void) {
  pthread_mutex_lock(&g_curl_pool_mutex);
  for (int i = 0; i < CURL_POOL_SIZE; i++) {
    if (g_curl_pool[i].handle) {
...
static cJSON *new_page_request(int page, int page_size) {
  ...
  cJSON_AddNumberToObject(req, "pageSize",
                          page_size > 0 ? page_size : LINYAPS_REMOTE_PAGE_SIZE);
  add_system_arch(req);
  return req;
}
...
static LinyapsRemoteAppInfo *parse_record(const cJSON *obj) {
  ...
  if (!info->base.name)
    info->base.name = jdup(obj, "appName");
  if (!info->base.name)
  ...
  if (!info->base.version)
    info->base.version = jdup(obj, "appVersion");
  ...
  if (!info->base.description)
    info->base.description = jdup(obj, "appDesc");
  ...
  if (info->base.size == 0)
    info->base.size = jint64(obj, "packageSize");

  /* remote-only fields */
  info->icon_url = jdup(obj, "icon");
  if (!info->icon_url)
    info->icon_url = jdup(obj, "appIcon");
  ...
  if (!info->base.create_time)
    info->base.create_time = jdup(obj, "updateTime");
  info->base.download_count = jint64(obj, "installCount");
  if (info->base.download_count == 0)
    info->base.download_count = jint64(obj, "downloadTimes");
...
static LinyapsRemoteAppInfo **remote_fetch_page(cJSON *req, const char *url,
                                                const char *tag,
                                                size_t *out_count,
                                                long *out_total) {
  if (out_count)
...
LinyapsRemoteAppInfo **linyaps_remote_fetch_apps(const char *keyword,
  ...
  char url[256];
  snprintf(url, sizeof(url), "%s/visit/getSearchAppList",
           LINYAPS_REMOTE_BASE_URL);
...
LinyapsRemoteAppInfo **linyaps_remote_fetch_welcome_apps(int page,
  ...
  char url[256];
  snprintf(url, sizeof(url), "%s/visit/getWelcomeAppList",
           LINYAPS_REMOTE_BASE_URL);
...
LinyapsRemoteAppInfo **linyaps_remote_fetch_ranking(LinyapsRankingType type,
  ...
  if (type == LINYAPS_RANKING_NEWEST) {
    snprintf(url, sizeof(url), "%s/visit/getNewAppList",
             LINYAPS_REMOTE_BASE_URL);
  } else {
    snprintf(url, sizeof(url), "%s/visit/getInstallAppList",
             LINYAPS_REMOTE_BASE_URL);
  }
...
linyaps_remote_check_updates(const LinyapsPackageInfo **installed_apps,
  ...
  if (out_count)
    *out_count = 0;
  if (!installed_apps || installed_count == 0)
    return NULL;
  ...
  LOG_DEBUG("remote", "POST %s/app/appCheckUpdate body=%s",
            LINYAPS_REMOTE_BASE_URL, body);
...
void linyaps_remote_app_info_list_free(LinyapsRemoteAppInfo **list,
  ...
  for (size_t i = 0; i < count; i++)
    linyaps_remote_app_info_free(list[i]);
  free(list);
}
