...
void pending_push(LinyapsContext *ctx, LinyapsProgressCallback cb,
                  void *userdata) {
...
int pending_pop(LinyapsContext *ctx, LinyapsProgressCallback *cb,
                void **userdata) {
...
int read_task_properties(LinyapsContext *ctx, TaskEntry *te) {
  ...
  r = sd_bus_get_property(ctx->bus, PM_DEST, te->path, TASK_IFACE, "State",
                          &err, &reply, "i");
  if (r >= 0) {
  ...
  r = sd_bus_get_property(ctx->bus, PM_DEST, te->path, TASK_IFACE, "Percentage",
                          &err, &reply, "d");
  if (r >= 0) {
  ...
  r = sd_bus_get_property(ctx->bus, PM_DEST, te->path, TASK_IFACE, "Message",
                          &err, &reply, "s");
  if (r >= 0) {
  ...
  r = sd_bus_get_property(ctx->bus, PM_DEST, te->path, TASK_IFACE, "Code", &err,
                          &reply, "i");
  if (r >= 0) {
...
static int read_additional_message(sd_bus_message *m,
                                   LinyapsInteractionRequest *request) {
...
static int on_request_interaction(sd_bus_message *m, void *userdata,
  ...
  if (message_id == 1 && request.local_ref && request.remote_ref) {
    request.body = dupstr("The lower version is installed. Continue installing "
                          "the newer version?");
  } else {
...
int on_task_added(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
  ...
  snprintf(match, sizeof(match),
           ...
           TASK_IFACE, path);
  r = sd_bus_add_match(ctx->bus, &te->interaction_slot, match,
                       on_request_interaction, te);
  if (r < 0) {
...
int on_task_removed(sd_bus_message *m, void *userdata,
                    sd_bus_error *ret_error) {
  (void)ret_error;
  LinyapsContext *ctx = userdata;
  const char *path = NULL;
...
int on_search_finished(sd_bus_message *m, void *userdata,
                       sd_bus_error *ret_error) {
  (void)ret_error;
  LinyapsContext *ctx = userdata;
  ...
  if (op->cb) {
    op->cb(r < 0 || code != 0 ? NULL : items, r < 0 || code != 0 ? 0 : count,
           r < 0 ? r : (int)code, r < 0 ? strerror(-r) : message, op->userdata);
  }
  if (r < 0 || code != 0 || !op->cb) {
