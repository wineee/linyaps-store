...
int append_package(sd_bus_message *m, const char *id, const char *version,
                   const char *channel, const char *module,
                   int include_module) {
  int r = sd_bus_message_open_container(m, 'e', "sv");
  if (r < 0)
    return r;
...
int append_options(sd_bus_message *m, int include_force, int force,
                   int include_skip) {
  int r = sd_bus_message_open_container(m, 'e', "sv");
  if (r < 0)
    return r;
...
int parse_job_info(sd_bus_message *reply, int64_t *code, char **message,
                   char **job_id) {
  int r;
  if (code)
    *code = 0;
...
int read_variant_first_string_array(sd_bus_message *m, const char *sig,
                                    char **out) {
...
int parse_package_map(sd_bus_message *m, const char *repo,
...
int parse_search_reply(sd_bus_message *reply, LinyapsPackageInfo ***out_items,
                       size_t *out_count, int64_t *out_code,
                       char **out_message) {
  ...
  while ((r = sd_bus_message_enter_container(reply, 'e', "sv")) > 0) {
    ...
    } else if (strcmp(key, "packages") == 0 && sig &&
               strcmp(sig, "a{sv}") == 0) {
      r = sd_bus_message_enter_container(reply, 'v', "a{sv}");
      if (r >= 0)
        r = sd_bus_message_enter_container(reply, 'a', "{sv}");
      while (r >= 0 &&
             (r = sd_bus_message_enter_container(reply, 'e', "sv")) > 0) {
        ...
        while (r >= 0 &&
               (r = sd_bus_message_enter_container(reply, 'v', "a{sv}")) > 0) {
...
static bool same_package_slot(const LinyapsPackageInfo *a,
                              const LinyapsPackageInfo *b) {
