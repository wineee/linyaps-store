...
static void test_context_lifecycle(void) {
  ...
  if (!ctx) {
    puts("system D-Bus unavailable; skipping context lifecycle integration "
         "check");
    return;
  }
...
static void test_state_string(void) {
  assert(strcmp(linyaps_task_state_string(LINYAPS_TASK_STATE_UNKNOWN),
                "Unknown") == 0);
  assert(strcmp(linyaps_task_state_string(LINYAPS_TASK_STATE_SUCCEED),
                "Succeed") == 0);
  assert(strcmp(linyaps_task_state_string(LINYAPS_TASK_STATE_CANCELED),
                "Canceled") == 0);
}
