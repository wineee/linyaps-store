...
static void print_package(const LinyapsPackageInfo *info, size_t index) {
  printf("%3zu  %-42s  %-12s  %-8s  %s\n", index, info->id ? info->id : "",
         info->version ? info->version : "", info->channel ? info->channel : "",
         info->name ? info->name : "");
}
...
static void search_cb(LinyapsPackageInfo **results, size_t count,
                      int error_code, const char *error_msg, void *userdata) {
...
static void progress_cb(const LinyapsTaskProgress *progress, void *userdata) {
  ...
  if (progress->object_path) {
    snprintf(state->last_task, sizeof(state->last_task), "%s",
             progress->object_path);
  }
...
static void interaction_cb(const LinyapsInteractionRequest *request,
                           void *userdata) {
...
int main(void) {
  ...
  for (;;) {
    ...
    char *argv[5] = {0};
    int argc = 0;
    for (char *tok = strtok(cmd, " \t"); tok && argc < 5;
         tok = strtok(NULL, " \t")) {
      argv[argc++] = tok;
    }
    if (argc == 0) {
    ...
    if (strcmp(argv[0], "quit") == 0 || strcmp(argv[0], "exit") == 0) {
      ...
      int pending = 1;
      linyaps_search(ctx, argv[1], argc >= 3 ? argv[2] : NULL, search_cb,
                     &pending);
      while (pending) {
        linyaps_process(ctx);
        const struct timespec delay = {.tv_sec = 0, .tv_nsec = 100000000L};
      ...
      state.active = 1;
      linyaps_install(ctx, argv[1], argc >= 3 ? argv[2] : NULL,
                      argc >= 4 ? argv[3] : "main", progress_cb, &state);
    } else if (strcmp(argv[0], "uninstall") == 0 && argc >= 3) {
      state.active = 1;
      linyaps_uninstall(ctx, argv[1], argv[2], argc >= 4 ? argv[3] : "main",
                        progress_cb, &state);
