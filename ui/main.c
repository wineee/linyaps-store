...
static void *fetch_remote_thread(void *arg) {
  ...
  if (category_id && strcmp(category_id, "__welcome__") == 0) {
    ...
    remote = linyaps_remote_fetch_apps(keyword ? keyword : "", category_id,
                                       page, page_size, &count, &total);
  }
  ...
  LOG_INFO("remote",
           "远端拉取完成: keyword=%s category=%s page=%d count=%zu total=%ld",
           keyword ? keyword : "", category_id ? category_id : "NULL", page,
           count, total);
...
static int current_items_per_page(bool has_pagination) {
  ...
  if (g_store->active_nav == NAV_ALL ||
      ...
      (g_store->active_nav >= NAV_CAT_OFFICE &&
       g_store->active_nav <= NAV_CAT_GAMES)) {
  ...
  if (has_pagination)
    content_h -= GRID_PAGE_H; /* Reserve space for pagination buttons */
  if (content_w < 1)
    content_w = 1;
  if (content_h < CARD_H)
    content_h = CARD_H;
  ...
  if (max_fit_cols > GRID_MAX_COLS)
    max_fit_cols = GRID_MAX_COLS;
  if (max_fit_cols < 1)
    max_fit_cols = 1;
  ...
  for (int cols = 1; cols <= max_fit_cols; cols++) {
    ...
    if (w > GRID_MAX_CARD_W)
      score += (w - GRID_MAX_CARD_W) * 2;
    int best_w = (content_w - gap * (best_cols - 1)) / best_cols;
    int best_score_val = abs(best_w - GRID_TARGET_CARD_W);
    if (best_w > GRID_MAX_CARD_W)
      best_score_val += (best_w - GRID_MAX_CARD_W) * 2;
...
static void start_remote_fetch(const char *keyword, const char *category_id,
                               int page) {
  ...
  farg->page = page;
  farg->page_size =
      current_items_per_page(true); /* Assume pagination for initial fetch */
...
static void *fetch_ranking_thread(void *arg) {
  ...
  size_t count = 0;
  long total = 0;
  LinyapsRankingType rtype =
      (tab_type == 0) ? LINYAPS_RANKING_NEWEST : LINYAPS_RANKING_DOWNLOADS;

  LinyapsRemoteAppInfo **remote =
      linyaps_remote_fetch_ranking(rtype, page, page_size, &count, &total);
...
static void on_update_progress(const LinyapsTaskProgress *prog,
                               void *userdata) {
...
static void *check_updates_real_thread(void *arg) {
  ...
  LinyapsPackageInfo **installed =
      linyaps_list_installed(g_store->ctx, &installed_count);
  ...
  LOG_INFO("updates", "已安装 %zu 个应用，调用批量检查更新 API",
           installed_count);
  ...
  if (remote && remote_count > 0) {
    ...
    if (updates) {
      for (size_t i = 0; i < remote_count; i++) {
        ...
        if (!remote_app || !remote_app->base.id || !remote_app->base.version)
          continue;

        /* O(1) 查找对应的已安装应用获取当前版本 */
        const char *current_ver =
            id_map_get(&installed_versions, remote_app->base.id);

        StoreUpdateItem *item = &updates[update_count];
        item->id = strdup(remote_app->base.id);
        item->name = strdup(remote_app->base.name ? remote_app->base.name
                                                  : remote_app->base.id);
        item->current_version = strdup(current_ver ? current_ver : "0.0.0");
        item->new_version = strdup(remote_app->base.version);
        item->channel = strdup(
            remote_app->base.channel ? remote_app->base.channel : "main");
        SDL_SetAtomicInt(&item->updating, 0);
        SDL_SetAtomicInt(&item->progress_int, 0);
        item->task_path = NULL;
        update_count++;

        LOG_INFO("updates", "发现更新: %s %s -> %s", remote_app->base.id,
                 current_ver ? current_ver : "0.0.0", remote_app->base.version);
      }
    }
...
void store_ui_trigger_check_updates(void) {
  if (!g_store || SDL_GetAtomicInt(&g_store->checking_updates))
    return;
...
void store_ui_trigger_update_item(StoreUpdateItem *item) {
  if (!g_store || !item || SDL_GetAtomicInt(&item->updating))
    return;
    ...
    for (size_t i = 0; i < g_store->installed_count; i++) {
      if (g_store->installed_list[i] && g_store->installed_list[i]->id &&
        ...
        linyaps_update(g_store->ctx, item->id,
                       g_store->installed_list[i]->channel, on_update_progress,
                       item);
        dispatched = true;
        break;
      }
...
void store_ui_trigger_change_nav(NavItem item) {
  if (g_store->active_nav == item && !g_store->is_searching)
    return;
...
void store_ui_trigger_change_ranking_tab(int tab_idx) {
  if (g_store->ranking_tab == tab_idx)
    return;
...
void store_ui_trigger_change_ranking_page(int page_idx) {
  ...
  if (g_store->ranking_list) {
    linyaps_package_info_list_free(g_store->ranking_list,
                                   g_store->ranking_count);
...
static const char *current_remote_category(void) {
  if (g_store->active_nav == NAV_RECOMMENDED)
    return "__welcome__";
  if (g_store->active_nav == NAV_ALL)
    return get_category_id_by_tab(g_store->active_cat);
  if (g_store->active_nav >= NAV_CAT_OFFICE &&
      g_store->active_nav <= NAV_CAT_GAMES)
    return get_category_id_by_nav(g_store->active_nav);
  return NULL;
}
...
static void on_search_results(LinyapsPackageInfo **items, size_t count,
                              int error_code, const char *message,
                              void *userdata) {
  StoreState *s = userdata;
  if (error_code != 0)
    LOG_WARN("search", "搜索失败: code=%d msg=%s", error_code,
             message ? message : "");
  else
  ...
  if (error_code == 0 && items) {
    ...
    s->current_page = 0;
    s->is_searching = true;
    strncpy(s->last_search_keyword, s->search_buf,
            sizeof(s->last_search_keyword) - 1);
  } else {
...
static bool handle_user_event(StoreState *store, const SDL_Event *e) {
  ...
  case EVT_REMOTE_READY: {
    ...
    char *data_str =
        e->user.data2; /* keyword (if search) or category_id (if browse) */
    if (res) {
      bool matches = false;
      if (res->is_search) {
        ...
        LOG_INFO("remote",
                 "搜索结果到达: keyword=\"%s\" current_buf=\"%s\" page=%d "
                 "current_page=%d matches=%d",
                 data_str ? data_str : "", store->search_buf, res->page,
                 store->current_page, matches);
      } else {
        /* Category browse: match if category still matches current nav */
        const char *current_cat = current_remote_category();
        matches =
            (!data_str && !current_cat) ||
      ...
      if (matches && store->current_page != res->page - 1) {
        LOG_INFO("remote", "页码不匹配: expected=%d got=%d",
                 store->current_page, res->page - 1);
        matches = false;
      }

      if (matches) {
        if (store->search_results != store->installed_list)
          linyaps_package_info_list_free(store->search_results,
                                         store->search_count);
        store->search_results = res->list;
        store->search_count = res->count;
        store->remote_total = res->total;
        store->last_page_data_count =
            (int)res->count; /* Cache the data count */
        if (res->is_search) {
          store->is_searching = true;
          strncpy(store->last_search_keyword, data_str,
                  sizeof(store->last_search_keyword) - 1);
        }
        SDL_SetAtomicInt(&store->loading_remote, 0);
        SDL_SetAtomicInt(&store->dirty, 1);
        LOG_INFO("remote", "更新 search_results: count=%zu total=%ld ptr=%p",
                 res->count, res->total, (void *)res->list);
        for (size_t i = 0; i < res->count && i < 5; i++) {
          LOG_INFO("remote", "  [%zu] id=%s name=%s", i, res->list[i]->id,
                   res->list[i]->name);
        }
        ...
        if (store->need_refetch_on_load_complete) {
          ...
          LOG_DEBUG(
              "event",
              "Window resized during load, re-fetching with new page size");
          if (store->is_searching) {
            start_remote_fetch(store->search_buf, NULL, 1);
                     ...
                     (store->active_nav >= NAV_CAT_OFFICE &&
                      store->active_nav <= NAV_CAT_GAMES)) {
    ...
    if (res) {
      if (store->ranking_tab == tab_type && store->current_page == (page - 1)) {
        if (store->ranking_list)
          linyaps_package_info_list_free(store->ranking_list,
                                         store->ranking_count);
  ...
  case EVT_CHECK_UPDATES_READY: {
    ...
    if (result) {
      ...
      free(result);
      LOG_INFO("updates", "更新检查完成，发现 %zu 个可更新应用",
               store->update_count);
    } else {
...
static bool handle_sdl_event(StoreState *store, KilnUI *ui, SDL_Event *e,
                             InputState *input) {
  if (e->type == SDL_EVENT_QUIT)
    return false;
  if (e->type == SDL_EVENT_KEY_DOWN && e->key.key == SDLK_ESCAPE &&
      !store->search_focused)
    return false;
  ...
  case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
     ...
     * KilnUI_handle_event 已经更新了 dpi_scale 和 mouse_scale */
    {
      ...
      if (window_w != store->window_w || window_h != store->window_h) {
        LOG_DEBUG("event", "Window resized: %dx%d -> %dx%d", store->window_w,
                  store->window_h, window_w, window_h);
        ...
        /* Only re-fetch if window got bigger and we don't have enough cached
         * data */
...
int main(int argc, char *argv[]) {
  ...
  if (!font) {
    SDL_Log("No font found");
    return 1;
  }
  if (!KilnUI_init(&ctx, "玲珑应用商店", 2556, 1478, font, 14))
    return 1;
  ...
  if (lctx) {
    ...
    id_set_build_from_packages(&store.installed_id_set, store.installed_list,
  ...
  while (running) {
    ...
    /* Render */
    if (SDL_GetAtomicInt(&store.dirty)) {
      UI_SetPointerState(input.mouse_down, input.mouse_released, input.mx,
                         input.my);
