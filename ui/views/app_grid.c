...
static void app_card(int card_idx, const LinyapsPackageInfo *info,
                     bool installed, int card_w) {
  int base = ID_CARD_BASE + card_idx * 20;

  Clay_ElementId card_id =
      Clay_GetElementIdWithIndex(CLAY_STRING("AppCard"), card_idx);
  ...
  CLAY(card_id,
    ...
    if (installed) {
      ...
      if (UI_Button(base + 5, "安装", UI_BTN_PRIMARY, UI_BTN_SM, false)) {
        ...
        LOG_INFO("ui", "安装按钮点击: id=%s ver=%s", app_id,
  ...
  CLAY(CLAY_SIDI(CLAY_STRING("SkelCard"), uid),
    ...
    CLAY(CLAY_SIDI(CLAY_STRING("SkelText"), uid + 2),
      ...
      CLAY(CLAY_SIDI(CLAY_STRING("SkelTitle"), uid + 3),
           {
               .layout = {.sizing = {CLAY_SIZING_PERCENT(0.6f),
      ...
      CLAY(CLAY_SIDI(CLAY_STRING("SkelDesc"), uid + 4),
           {
               .layout = {.sizing = {CLAY_SIZING_PERCENT(0.9f),
...
void view_app_grid(void) {
  if (SDL_GetAtomicInt(&g_state->loading_remote)) {
    ...
    if (g_state->active_nav == NAV_ALL ||
        ...
        (g_state->active_nav >= NAV_CAT_OFFICE &&
  ...
  if (!list || count == 0) {
    CLAY(CLAY_SIDI(CLAY_STRING("EmptyState"), ID_STATUS),
      ...
      TY_TextColored(ID_STATUS + 3, "搜索或选择分类以查看应用", TY_BODY,
                     ds_theme->muted);
    }
    return;
  }
  ...
  if (use_server_paging) {
    total_pages =
        (int)((g_state->remote_total + items_per_page - 1) / items_per_page);
  } else {
  ...
  if (g_state->current_page >= total_pages)
    g_state->current_page = total_pages - 1;
  if (g_state->current_page < 0)
    g_state->current_page = 0;

  /* For server-side paging, the fetched data IS the current page (no
   * sub-slicing) */
  /* Limit display to items_per_page even if we have more cached data */
  size_t start_idx = 0;
  size_t end_idx = count;
  if (end_idx > (size_t)items_per_page)
    end_idx = (size_t)items_per_page;
      ...
      while (i < end_idx) {
        UI_ROW(ID_STATUS + 20 + row_idx * 2, DS_SPACE_3) {
          ...
          for (; col < spec.cols && i < end_idx; col++, i++) {
            bool inst =
                (list[i]->id &&
                 id_set_contains(&g_state->installed_id_set, list[i]->id));
            app_card((int)i, list[i], inst, spec.card_w);
          }
          for (; col < spec.cols; col++) {
            CLAY(
                CLAY_SIDI(CLAY_STRING("CardFill"), ID_STATUS + 50 + col),
                {
    ...
    if (show_pagination) {
      CLAY(CLAY_SIDI(CLAY_STRING("PaginationBar"), ID_STATUS + 81),
        ...
        if (UI_Button(ID_STATUS + 83, "上一页",
                      prev_disabled ? UI_BTN_GHOST : UI_BTN_SECONDARY,
                      UI_BTN_SM, prev_disabled)) {
          if (!prev_disabled) {
            if (use_server_paging)
        ...
        CLAY(CLAY_SIDI(CLAY_STRING("PageText"), ID_STATUS + 84),
          ...
          CLAY_TEXT(UI__str(page_text),
                    {.textColor = ds_theme->text, .fontSize = DS_FS_SM});
        }

        bool next_disabled = (g_state->current_page >= total_pages - 1);
        if (UI_Button(ID_STATUS + 85, "下一页",
                      next_disabled ? UI_BTN_GHOST : UI_BTN_SECONDARY,
                      UI_BTN_SM, next_disabled)) {
          if (!next_disabled) {
            if (use_server_paging)
              store_ui_trigger_remote_page(g_state->current_page + 1);
            else {
