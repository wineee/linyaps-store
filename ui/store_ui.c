...
const char *store_ui_frame_str(const char *fmt, ...) {
  ...
  if (remaining <= 0) {
    ...
    if (!warned) {
      LOG_WARN("ui",
               "Frame string arena exhausted (%zu bytes). "
               "Consider increasing g_string_arena size.",
  ...
  if (n >= remaining) {
    ...
    if (!trunc_warned) {
      LOG_WARN("ui", "Frame string truncated: needed %d bytes, had %d", n + 1,
               remaining);
      trunc_warned = true;
    }
  }
...
void store_ui_reset_frame_str_arena(void) { g_string_arena_offset = 0; }
...
void launch_app(const char *app_id) {
  ...
  for (const char *p = app_id; *p; p++) {
    if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ||
        ...
        *p == '/')
      continue;
    LOG_WARN("launch", "app_id '%s' contains unsafe char '%c', aborting",
             app_id, *p);
    return;
  }
...
static void content_area(void) {
  CLAY(CLAY_SIDI(CLAY_STRING("ContentArea"), ID_ROOT + 2),
    ...
    if (g_state->active_nav == NAV_UPDATES) {
               ...
               .layout =
                   {
                       .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)},
                       .layoutDirection = CLAY_TOP_TO_BOTTOM,
                       .padding = {DS_SPACE_4, DS_SPACE_4, DS_SPACE_4,
                                   DS_SPACE_4},
                       .childGap = DS_SPACE_3,
                   },
      ...
      if (is_sidebar_cat) {
        CLAY(
            CLAY_SIDI(CLAY_STRING("CatHeader"), ID_STATUS + 400),
            {
                .layout =
                    {
          ...
          const char *hdr =
              store_ui_frame_str("%s (%zu)", NAV_LABELS[g_state->active_nav],
                                 g_state->search_count);
          CLAY_TEXT(UI__str(hdr),
                    {.textColor = ds_theme->text, .fontSize = DS_FS_LG});
        }
      } else if (g_state->active_nav == NAV_RECOMMENDED &&
                 !g_state->is_searching) {
        CLAY(
            CLAY_SIDI(CLAY_STRING("RecommendHeader"), ID_STATUS + 410),
            {
                .layout =
                    {
                        .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(44)},
      ...
      if (g_state->is_searching) {
        CLAY(CLAY_SIDI(CLAY_STRING("SearchHeader"), ID_STATUS + 420),
          ...
          const char *hdr =
              store_ui_frame_str("找到 %zu 个结果", g_state->search_count);
          CLAY_TEXT(UI__str(hdr),
...
void store_ui_handle_pre_layout_actions(StoreState *state,
                                        bool mouse_released) {
  if (!state || !mouse_released)
    return;

  Clay_ElementId theme_id =
      Clay_GetElementIdWithIndex(CLAY_STRING("UIIconBtn"), ID_THEME_TOGGLE);
