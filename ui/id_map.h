...
/* Insert or update a key-value pair. Both key and value are duplicated
 * internally. Returns true on success. */
 ...
 * The returned pointer is owned by the map and valid until the entry is
 * removed. */
...
void id_map_build_from_packages(IdMap *map, struct LinyapsPackageInfo **list,
                                size_t count);
